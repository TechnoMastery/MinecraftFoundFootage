...
public class Level0IntercomBasic extends AbstractEvent {
    ...
    public void init(World world) {
        ...
        if (intercomCount <= 1) {
            ...
            if (rand == 1) playSoundWithRandLocation(world, ModSounds.INTERCOM_BASIC1, 25, 20);
            else playSoundWithRandLocation(world, ModSounds.INTERCOM_BASIC2, 25, 20);
        }
        else if (intercomCount == 2) playSoundWithRandLocation(world, ModSounds.INTERCOM_REVERSED, 25, 20);
            ...
            else playSoundWithRandLocation(world, ModSounds.INTERCOM_REVERSED, 25, 20);
        }
    ...
    public void ticks(int ticks, World world) {
        ...
        if (friend) {
            if (ticks == 460) level.setLightState(BackroomsLevelWithLights.LightState.FLICKER);
            else if (ticks == 528) {
                level.setLightState(BackroomsLevelWithLights.LightState.OFF);
                playSound(world, ModSounds.LIGHTS_OUT);
            }
            else if (ticks == 656) level.setLightState(BackroomsLevelWithLights.LightState.ON);
        }
    }
