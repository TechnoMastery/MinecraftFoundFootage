...
public class Level0Music extends AbstractEvent {
    ...
    public void init(World world) {
        ...
        if (intercomCount > 2) {
            ...
            if (rand == 1) playSoundWithRandLocation(world, ModSounds.CREEPY_MUSIC1, 25, 20);
            else if (rand == 2) playSoundWithRandLocation(world, ModSounds.CREEPY_MUSIC2, 25, 20);
            else playSoundWithRandLocation(world, ModSounds.FAR_CROWD, 25, 20);
        }
        else {
            if (level.getIntercomCount() == 2) playSoundWithRandLocation(world, ModSounds.INTERCOM_REVERSED, 25, 20);
            else {
